...
void CHAS_Remote_Ctrl(void)
{
  ...
  if(abs(Chassis.RC_Move.Z_Speed) > 0)
  {
    ...
    kZ_Adapt = kZ_Adapt * kZ_Adapt * kZ_Adapt;//4次方
...
//        Chassis.Key_Move.Z_Speed = Chassis.Key_Move.Z_Speed_k   * CHAS_Park_Z_Speed();
  ...
  if(abs(Chassis.Key_Move.Z_Speed) > 0 && Chassis.PID_type != ACT_PARK)
  {
    ...
    kZ_Adapt = kZ_Adapt * kZ_Adapt;//2次方
    kZ_Adapt = constrain(kZ_Adapt, 1.f, 0.f);
  } else kZ_Adapt = 1;
...
//重心偏移修正算法
float WHEELBASE   = 358  ;   //前后轮距
float WHEELTRACK  = 382  ;   //左右轮距
float GIMBAL_X_OFFSET =  15 ;  //前后偏移
float GIMBAL_Y_OFFSET =  6  ;    //左右偏移
//float RADIAN_COEF 57.3f//重心偏移算法的弧度系数
...
{//比赛功率算法
  ...
  switch(IF_DUM_NORMAL)
  {
    case YES:
        ...
        //SuperCap_Emergency(str->Real_PowerBuffer);//紧急超电功能
    ...
    case KEY_CTRL_MODE:
        ...
        if(!Chassis.IF_SPIN_CLOSING) {
          ...
          { 
            ...
            case Defense_BIG_DODGE:
                Chassis.SPIN_Move.FB_Speed += 1000 * Chassis.Dir.num; //偏心转
...
void SPIN_CLOSING_Judge(void)
{
  ...
  {   
    ...
    if(abs(YAW_AngleErrReport()) <= 20)//到位时间
      Angleback_cnt++;
    closing_cnt++;//“卡住”时间
 ...
 * @param 
 */
uint8_t SuperCap_cmd = 0;//主动超电开关
uint8_t SuperCapUrgent_cmd = 0;//被动超电开关
...
void SuperCap_Urgent_ON(void)//紧急超电开
{
...
void SuperCap_Urgent_OFF(void)//紧急超电关
{
